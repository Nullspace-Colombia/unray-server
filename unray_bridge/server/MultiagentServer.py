...
if __name__ == "__main__":
    ...
    """
    register_env("multi_agent_cartpole", lambda config: DummyMultiAgentEnv(config))

    num_agents_in_env = 2
    server_port = 5555  # Base port
    ...
    config = (
        ...
        .environment(
            ...
            env_config={"num_agents": num_agents_in_env, "port": server_port}
        )
        .env_runners(
            env_runner_cls=TcpClientMultiAgentEnvRunner,
        ...
        .multi_agent(
            ...
            policy_mapping_fn=lambda agent_id, episode, worker, **kwargs: policy_ids[
                int(agent_id.split('_')[1]) % len(policy_ids)],
            # Optionally, specify which policies to train
            policies_to_train=[policy_ids[0], policy_ids[1]],
        )
        .framework("torch")
        .debugging(
            log_level="INFO",
